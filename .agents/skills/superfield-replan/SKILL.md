...
Use this skill to rewrite the `Plan` tracking issue around phase-aware
delivery with scout-first execution inside each phase.

This skill owns orchestration. Deterministic scripts handle compliance,
...
- Do not designate a feature issue as a phase scout. Scouts must be issues
  whose `## Issue type` is `dev-scout`. If no such issue exists for a phase,
  the `replan-evaluate` skill emits a null-numbered scout spec and
  `create-scout-issues.sh` creates the real issue after phase scope is decided.
...
4. Create any missing scout issues, then apply the result:

```bash
.agents/scripts/replan/create-scout-issues.sh {plan-json-file}
