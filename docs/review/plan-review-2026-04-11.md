...
<!-- cross-checked: docs/PRD.md, docs/technical/db-architecture.md, ~/calypso-distribution -->
...
- `calypso-distribution/k8s/app.yaml` already declares `DATABASE_URL`,
...
`calypso-distribution` already has the full implementation:
...
**Fix:** Add Phase 0 follow-on: _"task queue scaffold — reuse calypso-distribution
...
### C3. Dev environment is Docker Compose; blueprint and calypso-distribution both use k3d
...
`calypso-distribution` already uses **k3d** for local demo:
...
**Fix:** Phase 0 uses k3d from day one, reusing `calypso-distribution/scripts/local-demo.ts`
and `calypso-distribution/deploy/base/` manifests as the starting point. Remove Docker Compose.
...
`calypso-distribution` already has 14 GitHub workflows (test-unit, test-api, test-component,
...
| M6  | Production deployment human-action gate not stated                   | `DEPLOY-D-005`, `DEPLOY-C-027`                  | State explicitly; `calypso-distribution/deploy.yml` already gates prod on Environment approval                   |
...
| M10 | Worker network policy (pod→DB) not specified                         | `WORKER-C-006`                                  | Phase 3 scout: `NetworkPolicy` blocking worker pod → DB port; reuse `calypso-distribution/k8s/agent-worker.yaml` |
...
## calypso-distribution reuse opportunities

| Component                                             | Source in calypso-distribution                              | Target phase                      |
| ----------------------------------------------------- | ----------------------------------------------------------- | --------------------------------- |
...
2. Plan says "nine-check CI" but `calypso-replan` grades against twelve. Fix #5.
...
| ---------------------------------------------------------------------------------- | ----- | ---------------------------- |
| feat: k3d dev cluster scaffold reusing calypso-distribution manifests              | 0     | C3 — replaces Docker Compose |
| feat: task queue scaffold reusing calypso-distribution implementation              | 0     | C2                           |
...
| Issue | Change                                                                                                  |
| ----- | ------------------------------------------------------------------------------------------------------- |
| #5    | "nine-check CI gate" → "twelve-check CI gate"; reference calypso-distribution/`.github/workflows/`      |
| #6    | "docker compose stack" → "k3d cluster scaffold"; reference `calypso-distribution/scripts/local-demo.ts` |
