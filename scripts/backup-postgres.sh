#!/usr/bin/env bash
# backup-postgres.sh — Encrypted Postgres backup script for Calypso.
#
...
#   BACKUP_STORE_DIR    — Directory for backup artifacts (default: /var/backups/calypso)
...
BACKUP_STORE_DIR="${BACKUP_STORE_DIR:-/var/backups/calypso}"
...
DATABASE_URL="${DATABASE_URL:-postgres://app_rw:app_rw_password@localhost:5432/calypso_app}"
AUDIT_DATABASE_URL="${AUDIT_DATABASE_URL:-postgres://audit_w:audit_w_password@localhost:5432/calypso_audit}"

mkdir -p "${BACKUP_STORE_DIR}"

TIMESTAMP="$(date -u '+%Y-%m-%dT%H-%M-%SZ')"

echo "[backup] Starting Calypso Postgres backup — ${TIMESTAMP}"
