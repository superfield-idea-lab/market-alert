...
def is_rule_file(relpath: str) -> bool:
    """Return True when the repo-relative path is a rule YAML file."""
    if not relpath.startswith("rules/"):
        return False
    candidate = relpath[len("rules/"):]
    return any(fnmatch.fnmatch(candidate, pattern) for pattern in RULE_GLOBS)
...
def get_base_rules(base_branch: str) -> dict:
    ...
    numbers = {}  # number -> hash
    rules_by_number = {}  # number -> rule_dict

    try:
        ls_output = subprocess.check_output(
            ["git", "ls-tree", "-r", "--name-only", base_branch, "rules"],
    ...
    for fpath in ls_output.splitlines():
        if not fpath or not is_rule_file(fpath):
            continue
        try:
        ...
        for h, rule in file_rules.items():
            ...
            if num:
                numbers[num] = h
                rules_by_number[num] = rule

    return rules, numbers, rules_by_number
...
def get_current_rules() -> dict:
    ...
    for pattern in RULE_GLOBS:
        for fpath in sorted(glob.glob(os.path.join(RULES_DIR, pattern), recursive=True)):
...
def diff_rules(base_rules, base_numbers, base_rules_by_number, current_rules, current_numbers, current_rules_by_number):
    """Compute the diff between base and current rule sets."""
    base_hashes = set(base_rules.keys())
    current_hashes = set(current_rules.keys())
    hash_repairs = {}

    for num, base_hash in base_numbers.items():
        curr_hash = current_numbers.get(num)
        if not curr_hash or curr_hash == base_hash:
            continue

        base_rule = base_rules_by_number.get(num)
        curr_rule = current_rules_by_number.get(num)
        if not base_rule or not curr_rule:
            continue

        # Allow corrective hash changes for the same semantic rule identity.
        if (
            base_rule.get("number") == curr_rule.get("number")
            and base_rule.get("name") == curr_rule.get("name")
            and base_rule.get("type") == curr_rule.get("type")
        ):
            hash_repairs[num] = (base_hash, curr_hash)

    added_hashes = current_hashes - base_hashes
    removed_hashes = base_hashes - current_hashes
    common_hashes = base_hashes & current_hashes

    for old_hash, new_hash in hash_repairs.values():
        added_hashes.discard(new_hash)
        removed_hashes.discard(old_hash)
    ...
    for num, (old_hash, new_hash) in sorted(hash_repairs.items()):
        base = base_rules_by_number[num]
        curr = current_rules_by_number[num]
        changes = ["hash"]
        if base.get("description", "").strip() != curr.get("description", "").strip():
            changes.append("description")
        if base.get("links") != curr.get("links"):
...
def main():
    ...
    base_rules, base_numbers, base_rules_by_number = get_base_rules(base_branch)
    current_rules, current_numbers, current_rules_by_number = get_current_rules()
    ...
    added, removed, modified, newly_deprecated, reused = diff_rules(
        base_rules, base_numbers, base_rules_by_number, current_rules, current_numbers, current_rules_by_number
    )
