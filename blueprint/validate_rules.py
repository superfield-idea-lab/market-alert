...
all_files = sorted(
    glob.glob(os.path.join(RULES_DIR, "blueprints", "*.yaml")) +
    glob.glob(os.path.join(RULES_DIR, "implementations", "**", "*.yaml"), recursive=True)
)
file_data = {}
parse_errors = []
for f in all_files:
...
if in_graph_not_source:
    ...
    for h in sorted(in_graph_not_source, key=str)[:20]:
        node = graph["nodes"][h]
        print(f"    {h}: {node[0]}")
else:
...
if in_source_not_graph:
    ...
    for h in sorted(in_source_not_graph, key=str)[:20]:
        locs = all_hashes[h]
