...
Canonical blueprint docs now live under `calypso-blueprint/`; `agent-context/workflows/` continues to host this repo's custom workflow YAMLs.

## CURRICULUM

### P1: Orient

0. IF `calypso-blueprint/index.md` is missing or unreadable: RUN `git submodule update --init --remote calypso-blueprint` to hydrate the canonical blueprint docs.
1. READ `calypso-blueprint/index.md` (Document graph & keyword index).
...
Match task to EXACTLY ONE workflow in `calypso-blueprint/development/`. READ & STRICTLY FOLLOW it:
...
1. READ domain implementation doc (via Task Routing in `calypso-blueprint/index.md`).
...
2. READ `calypso-blueprint/index.md` keyword index.
